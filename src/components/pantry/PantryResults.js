    ...
    const fetchEntrees = async () => {
      try {
        ...
        const fetchData = await axios.get(
          `https://api.spoonacular.com/recipes/findByIngredients?apiKey=${process.env.REACT_APP_API_KEY}&ingredients=${queryStr}&number=1`
        );
            ...
            try {
              ...
              for await (const item of dataIdArr) {
                const recipeId = await axios.get(
                  `https://api.spoonacular.com/recipes/${item}/information?apiKey=${process.env.REACT_APP_API_KEY}&includeNutrition=false`
                );
          ...
          if (dataId.length === 1) {
            return (
              <div href={item.sourceUrl} target="_blank" key={item.id}>
                ...
                <h3>{item.title}</h3>
                <a
                  href={item.spoonacularSourceUrl}
                  target="_blank"
                  rel="noreferrer"
                >
